                ...
                <div class="px-4 pb-4">
                    <div class="pla-chart-card">
                        ...
                        <div id="grafico-empty" class="text-center text-muted py-5" style="display:none;">
                            ...
                            <p class="mb-0" style="font-size:.85rem;">Agrega documentos con totales mensuales para ver el gráfico.</p>
                        </div>
        ...
        <div class="modal-content pla-modal">
            ...
            <div class="modal-body p-4">
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-8">
                        <label class="pla-label">Archivos (puede seleccionar varios) <span class="text-danger">*</span></label>
                        <input type="file" class="pla-input" id="aArchivos" accept=".xlsx,.xls,.csv,.pdf" multiple>
                ...
                <div class="row g-3" id="divATotal" style="display:none;">
                    <div class="col-12">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="pla-input" id="aTotal"
                                   placeholder="Ej: 1.250.000" oninput="fmtMiles(this)">
                        </div>
        ...
        <div class="modal-content pla-modal">
            ...
            <div class="modal-body p-4">
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-8">
                        <label class="pla-label">Reemplazar archivo <small class="text-muted fw-400">(opcional)</small></label>
                    ...
                    <div class="row align-items-center">
                        <div class="col-2">
                            <lord-icon src="https://cdn.lordicon.com/oqdmuxru.json" trigger="loop"
                                       delay="2000" style="width:65px;height:65px;"></lord-icon>
                        </div>
...
@media (max-width: 991px) {
    .pla-row { grid-template-columns: 1fr; }
    .pla-row-header { display: none !important; }
}

.pla-row--disabled { opacity: .6; }
.pla-row--disabled .pla-cell { pointer-events: none; }
...
.pla-cell:hover { box-shadow: 0 6px 22px rgba(0,0,0,.11); transform: translateY(-2px); }
...
.pla-cell-title { font-weight: 700; font-size: .9rem; color: #1a1a2e; line-height: 1.25; }
.pla-cell-sub   { font-size: .72rem; color: #aaa; margin-top: 2px; }

/* Docs cell */
.pla-cell--docs { justify-content: space-between; }
...
.pla-doc-zone--disabled { cursor: not-allowed; opacity: .6; }

.pla-doc-list { width: 100%; }
.pla-doc-item {
    display: flex; align-items: center; gap: 8px;
    font-size: .78rem; color: #555;
    padding: 4px 0; border-bottom: 1px solid #f0ede8;
}
.pla-doc-item:last-child { border-bottom: none; }
.pla-doc-item i { color: #27AE60; }

/* ─── Total block ─── */
.pla-total-inner { flex: 1; }
.pla-tlabel { font-size: .7rem; color: #aaa; text-transform: uppercase; letter-spacing: .5px; }
.pla-tvalue {
...
.pla-tnota { font-size: .7rem; color: #bbb; }
.pla-to-chart {
...
.pla-to-chart i { color: #E67E22; }

/* ─── Botones de acción ─── */
.pla-actions { display: flex; gap: 8px; }
.pla-abtn {
    flex: 1; padding: 7px 10px; border-radius: 9px; border: 2px solid;
    background: transparent; font-size: .77rem; font-weight: 700;
    cursor: pointer; transition: all .2s;
    display: flex; align-items: center; justify-content: center; gap: 5px;
}
.pla-abtn:hover { opacity: .85; transform: translateY(-1px); }
.pla-abtn--filled { color: #fff !important; border-color: transparent !important; }
...
.pla-toggle:hover  { border-color: #E67E22; color: #E67E22; }
.pla-toggle.active { background: #E67E22; border-color: #E67E22; color: #fff; }
...
.pla-mini-label { font-size: .7rem; color: #aaa; text-transform: uppercase; letter-spacing: .4px; }
.pla-mini-value { font-size: .95rem; font-weight: 800; color: #1a1a2e; }
.pla-mini-raw   { font-size: .72rem; color: #bbb; margin-top: 1px; }
...
.pla-label {
    ...
    color: #555; text-transform: uppercase; letter-spacing: .4px; margin-bottom: 5px;
}
.pla-input {
...
.pla-input:focus {
    border-color: #E67E22 !important; outline: none;
...
.pla-btn-pri:hover { background: #D35400; }
.pla-btn-sec {
...
.pla-table thead th {
    ...
    font-size: .77rem; font-weight: 700; color: #666;
...
.pla-table tbody td {
    ...
    border-top: 1px solid #f5f2ee; border-bottom: 1px solid #f5f2ee;
}
.pla-table tbody td:first-child { border-left: 1px solid #f5f2ee; border-radius: 8px 0 0 8px; }
.pla-table tbody td:last-child  { border-right: 1px solid #f5f2ee; border-radius: 0 8px 8px 0; }
...
const CFG = {
    1: { titulo:'Planilla 10% Administración', color:'#E67E22', icono:'fa-percent',       sinTotal:false, esAdmin:true  },
    2: { titulo:'Planilla Arriendos Mensuales', color:'#2980B9', icono:'fa-key',           sinTotal:false, esAdmin:false },
    3: { titulo:'Planilla Ventas',              color:'#27AE60', icono:'fa-tag',           sinTotal:false, esAdmin:false },
    4: { titulo:'Planilla Obras',               color:'#8E44AD', icono:'fa-helmet-safety', sinTotal:false, esAdmin:false },
    6: { titulo:'Planilla Arqueo',              color:'#C0392B', icono:'fa-calculator',    sinTotal:true,  esAdmin:false },
};
...
$(document).on('click', '.btn-agregar', function () {
    ...
    if (c.sinTotal) {
        $('#divATotal').hide(); $('#divATotalLabel').hide();
    } else {
        $('#divATotal').show(); $('#divATotalLabel').show();
...
$('#btnGuardar').on('click', function () {
    ...
    if (!archivos.length) { alert('Selecciona al menos un archivo.'); return; }
    if (!sinTotal && !CFG[tipo].esAdmin && parseNum(totalStr) <= 0) {
        alert('Ingresa el total del mes.'); return;
    }
    ...
    $.ajax({
        url: '/planillas/guardar', type: 'POST', data: fd,
...
function onGuardadoFallback(tipo, archivos, fecha, sinTotal, totalStr, hide = true) {
    Array.from(archivos).forEach(f => {
        DB[tipo].push({
            ...
            fecha, nombre_archivo: f.name, documento: null,
            total_mes: sinTotal ? null : parseNum(totalStr), tipo,
        });
    });
    refreshCard(tipo);
...
function renderTabla(tipo) {
    ...
    if (!items.length) {
        tbody.html('<tr><td colspan="5" class="text-center py-5 text-muted"><i class="fa-solid fa-inbox fa-2x mb-2 d-block opacity-25"></i>No hay archivos registrados</td></tr>');
        return;
    }
    ...
    items.forEach((item, i) => {
        const totalCell = c.sinTotal
            ...
            : (item.total_mes != null ? `<span class="fw-700">${money(item.total_mes)}</span>` : '<span class="text-muted">—</span>');
...
$(document).on('click', '.btn-editar', function () {
    ...
    $('#eTotal').val(item.total_mes != null
        ? Number(item.total_mes).toLocaleString('es-CL', { maximumFractionDigits:0 }) : '');

    if (c.sinTotal) { $('#divETotal').hide(); $('#divETotalLabel').hide(); }
    else            { $('#divETotal').show(); $('#divETotalLabel').show(); $('#eAdminNota').toggle(c.esAdmin); }
...
$('#btnEditar').on('click', function () {
    ...
    $.ajax({
        url: '/planillas/editar', type: 'POST', data: fd,
        processData: false, contentType: false,
...
$('#btnConfEliminar').on('click', function () {
    ...
    $.ajax({
        url: '/planillas/eliminar/' + id, type: 'DELETE',
